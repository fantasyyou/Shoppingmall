function openreselect(id) 
 {  
	var allCitys=
		...
		{ "name": "重庆", "city":[
		{"name":"重庆", "area":["渝中区","大渡口区","江北区","南岸区","北碚区","渝北区","巴南区","长寿区","双桥区","沙坪坝区","万盛区","万州区","涪陵区","黔江区","永川区","合川区","江津区","九龙坡区","南川区","綦江县","潼南县","荣昌县","璧山县",
		 "大足县","铜梁县","梁平县","开县","忠县","城口县","垫江县","武隆县","丰都县","奉节县","云阳县","巫溪县","巫山县","石柱土家族自治县","秀山土家族苗族自治县","酉阳土家族苗族自治县","彭水苗族土家族自治县","其他"]}
		]},
		{ "name": "四川", "city":[
	...
	var length="";
	document.getElementById("re-select").style.display="block";  //开启选址区域
	if(id=="re-select-1")
	  { 
		var tag="";     //一级标签
	    for(var i=0;i<allCitys.length;i++)   //添加一级标签
	       tag=tag+"<div id="+i+" class='re-address-tag'>"+allCitys[i].name+"</div>";
	    document.getElementById("re-tag-choice").innerHTML=tag+"<div style='float:left;width:100%;height:20px;'></div>";
	    for(var i=0;i<allCitys.length;i++)   //给一级标签添加点击事件
	       document.getElementById(i).onclick=function(){getprovince(this)};
	  }
	if(id=="re-select-2")
	  {
		var tag="";     //二级标签
	    for(var i=0;i<allCitys.length;i++)   //添加二级标签
	      {
	    	if(document.getElementById("re-select-1").innerHTML==allCitys[i].name)
	    	  { 
	    		length=allCitys[i].city.length;
	    		for(var j=0;j<allCitys[i].city.length;j++)
	    		   tag=tag+"<div id="+j+" class='re-address-tag'>"+allCitys[i].city[j].name+"</div>";
	    		break;
	    	  }
	      }
	    document.getElementById("re-tag-choice").innerHTML=tag+"<div style='float:left;width:100%;height:20px;'></div>";
	    for(var i=0;i<length;i++)   //给二级标签添加点击事件
	    	document.getElementById(i).onclick=function(){getcity(this)};
	  }
	if(id=="re-select-3")
	  { 
		var tag="";     //三级标签
	    for(var i=0;i<allCitys.length;i++)   //添加三级标签
	      {
	        if(document.getElementById("re-select-1").innerHTML==allCitys[i].name)
	    	 { 
	    	   for(var j=0;j<allCitys[i].city.length;j++)
	             {
	    		   if(document.getElementById("re-select-2").innerHTML==allCitys[i].city[j].name)
	    			 {
		   	    	   length=allCitys[i].city[j].area.length;
			    	   for(var k=0;k<allCitys[i].city[j].area.length;k++)
			    		 tag=tag+"<div id="+k+" class='re-address-tag'>"+allCitys[i].city[j].area[k]+"</div>";
			    	   break;
	    			 }
	             }
	    	   break;
	    ...
	    document.getElementById("re-tag-choice").innerHTML=tag+"<div style='float:left;width:100%;height:20px;'></div>";
	    for(var i=0;i<length;i++)   //给三级标签添加点击事件
	    	document.getElementById(i).onclick=function(){getcounty(this)};
	  }
 }
function closereselect(id)
 {
   document.getElementById("re-select").style.display="none";
 }

function getprovince(tag)
 { 
   document.getElementById("re-select-1").innerHTML=tag.innerHTML;
   document.getElementById("re-select-2").style.display="block";
   document.getElementById("re-select-2").innerHTML="--请选择--";
   document.getElementById("re-select-3").style.display="none";
   openreselect("re-select-2");
 }

function getcity(tag)
 { 
   document.getElementById("re-select-2").innerHTML=tag.innerHTML;
   document.getElementById("re-select-3").style.display="block";
   document.getElementById("re-select-3").innerHTML="--请选择--";
   openreselect("re-select-3");
 }

function getcounty(tag)
 { 
   document.getElementById("re-select-3").innerHTML=tag.innerHTML;
   var province=document.getElementById("re-select-1").innerHTML;
   var city=document.getElementById("re-select-2").innerHTML;
   var county=document.getElementById("re-select-3").innerHTML;
   document.getElementById("re-address").innerHTML=province+"/"+city+"/"+county;
   document.getElementById("re-select").style.display="none";
 }